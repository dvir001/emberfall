...
public sealed class NanoChatCartridgeSystem : EntitySystem
{
        ...
        NanoChatMessage message)
    {
        ...
        if (recipient.Comp.Closed || _nanoChat.GetCurrentChat((recipient, recipient.Comp)) != senderNumber)
    ...
    private void UpdateUI(Entity<NanoChatCartridgeComponent> ent, EntityUid loader)
    {
        ...
        if (ent.Comp.Card is { } uid && TryComp<NanoChatCardComponent>(uid, out var card))
        {
